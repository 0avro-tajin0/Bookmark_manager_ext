  ...
  chrome.storage.sync.get(null, existing => {
    ...
    const done = () => {
      chrome.storage.sync.set(toSet).then(() => {
        ...
        console.warn('[Markmez] Sync write failed:', err.message);
...
function addBoardAt(col, row) {
  if (isGuest() && S.boards.filter(b => b.pageId === S.activePage).length >= 3) {
...
document.getElementById('mpImport').addEventListener('click', () => {
  if (isGuest()) { closeSidebar(); showLimitHint(T('limit.import')); return; }
...
function buildWallpaperBody() {
  ...
  zone.addEventListener('click', () => {
    if (isGuest()) { showLimitHint(T('limit.uploadWp')); return; }
...
document.getElementById('fileInput').addEventListener('change', async e => {
  ...
  e.target.value = '';
  if (isGuest()) { showLimitHint(T('limit.customWp')); return; }
...
async function signOutGoogle() {
  ...
  localStorage.removeItem('mz-onboard-done');
  localStorage.removeItem('mz-tour-done');
  showOnboarding();
}
...
function isPaid() {
  if (localStorage.getItem('mz-activated') !== '1') return false;
  const plan = localStorage.getItem('mz-plan') || 'lifetime';
  if (plan !== 'year') return true;
  const at = parseInt(localStorage.getItem('mz-activated-at') || '0', 10);
  if (!at) return true; // legacy/safety: no timestamp → treat as active
...
const ACTIVATION_HASH = 'ab27459da277b4e93b4f8833bb6657ad930c31a624d4895808a23a9c00b1e696';
...
async function _validateCode(code) {
  return await _hashCode(code) === ACTIVATION_HASH;
}
...
function updateTrialBadge() {
  ...
  if (!el) return;
  if (isGuest() || getTrialStatus() !== 'trial') { el.style.display = 'none'; return; }
...
function checkTrialNudge() {
  if (isGuest() || getTrialStatus() !== 'trial') return;
...
function exportBookmarks() {
  ...
  a.download = 'markmez-export.json';
  a.click();
  URL.revokeObjectURL(a.href);
}

function showLimitHint(msg) {
  track('signin_prompt_shown', { feature: msg });
...
function _closePlanChooser() {
  document.getElementById('trialOverlay').style.display = 'none';
  // Trial already over and still unpaid → the paywall is a hard gate, so bounce
  // back to it instead of leaking access to the app.
  if (!isPaid() && getTrialStatus() === 'expired') {
    document.getElementById('paywallOverlay').style.display = '';
  }
}
...
function _afterSignIn() {
  ...
  // Already picked a plan once (trial or purchase) → never show the chooser again.
  if (isPaid() || localStorage.getItem('mz-plan-chosen')) {
    if (!isPaid() && getTrialStatus() === 'expired') {
      track('paywall_shown', { reason: 'trial_expired' });
      document.getElementById('paywallOverlay').style.display = '';
    }
    return;
  }
...
function _limitSignInDoSignIn(btn) {
  btn.innerHTML = T('signin.inProgress'); btn.disabled = true;
  _trackSignin('limit', signInGoogle()).then(() => {
    _closeLimitSignIn();
    localStorage.setItem('mz-onboard-done', '1');
    _afterSignIn();
  }).catch(err => {
    console.error('[Markmez] Sign-in failed:', err);
    document.getElementById('limitSignInMain').style.display = 'none';
    document.getElementById('limitSignInError').style.display = '';
  });
}

document.getElementById('limitSignInGoogleBtn').addEventListener('click', e => _limitSignInDoSignIn(e.currentTarget));
document.getElementById('limitSignInRetryBtn').addEventListener('click', e => {
  document.getElementById('limitSignInError').style.display = 'none';
  document.getElementById('limitSignInMain').style.display = '';
  _limitSignInDoSignIn(document.getElementById('limitSignInGoogleBtn'));
});
...
document.getElementById('onboardGuestBtn').addEventListener('click', _continueAsGuest);
document.getElementById('onboardGuestBtn2').addEventListener('click', _continueAsGuest);
...
function renderSettingsBody() {
  ...
  if (S.user?.signedIn) {
    ...
    if (status === 'paid') {
      ...
      const statusRow = document.createElement('div');
      statusRow.className = 'st-plan-row';
      statusRow.innerHTML = `<span class="st-plan-badge expired">${T('st.trialEnded')}</span>`;
      planSec.appendChild(statusRow);
      planSec.appendChild(planButtons(closeSettingsModal));
    }
  } else {
    const guestNote = document.createElement('p');
    guestNote.className = 'st-plan-guest';
    guestNote.textContent = T('st.guestNote', { n: TRIAL_DAYS });
    planSec.appendChild(guestNote);
  }
...
async function init() {
  ...
  // Trial checks (only for signed-in users)
  if (!isGuest()) {
    if (!localStorage.getItem('mz-plan-chosen')) {
      // User signed in but hasn't picked a plan yet — show chooser
      document.getElementById('trialChooseState').style.display = '';
      document.getElementById('trialPurchaseState').style.display = 'none';
      document.getElementById('trialOverlay').style.display = '';
...
document.getElementById('paywallWaitBackBtn')?.addEventListener('click', () => {
  ...
  if (_checkoutFromTrial) {
    ...
    // Trial still active → just close, let user continue
    document.getElementById('paywallOverlay').style.display = 'none';
  } else {
    // Trial expired → return to paywall main screen
    document.getElementById('paywallMainState').style.display = '';
  }
});
...
document.getElementById('activationActivateBtn').addEventListener('click', async () => {
  ...
  if (valid) {
    const plan = localStorage.getItem('mz-pending-plan') || 'lifetime';
